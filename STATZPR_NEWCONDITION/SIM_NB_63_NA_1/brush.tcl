...
set N_B 63 	;#monomer units of main chain
...
for { set i 1 } { $i < $N_B } { incr i 1 } {
 ...
 if { $i>2 } { part [expr $i-1] bond 1 [expr $i-2] $i }
}
set NP $N_B
if { $N_A > 0 } {
 for { set i 1 } { $i <= $N_B } { incr i $h } {
  if { $N_A > 1 } {
   ...
   if {[expr $i-2]>=0} {part [expr $i-1] bond 1 [expr $i-2] $NP}
   if {[expr $i]<=$N_B} {part [expr $i-1] bond 1 [expr $i] $NP}
   incr NP 1
   for { set j 2 } { $j <= [expr $N_A-1] } { incr j 1 } {
    part $NP pos [expr $i*$r_0] [expr $j*$r_0] 0.0 bond 0 [expr $NP-1]
    if { $j>2 } {part [expr $NP-1] bond 1 [expr [expr $NP-2] $NP] 
    } else { part [expr $NP-1] bond 1 [expr [expr $i-1] $NP] }
    incr NP 1
   }
   part $NP pos [expr $i*$r_0] [expr $N_A*$r_0] 0.0 bond 0 [expr $NP-1] q 1.0
   if { $N_A>2 } {part [expr $NP-1] bond 1 [expr [expr $NP-2] $NP] 
    } else { part [expr $NP-1] bond 1 [expr [expr $i-1] $NP] }
   incr NP 1
  } else {
