...
set N_A 4 	;#monomer units of side chain
...
### TESTING SOLVENT
set Nc_s 0.001   ;# just for testing
...
set virt 	yes     ;#yes/no virtual particle
set move 	0.1	;#how much move
...
inter 3 harmonic $k_B 0
inter 0 0 lennard-jones $eps $sgm $Pr_c $Pc_s
inter 0 1 lennard-jones $eps $sgm $Pr_c $Pc_s
inter 1 1 lennard-jones $eps $sgm $Pr_c $Pc_s
inter 0 2 lennard-jones $eps $sgm $Pr_c $Pc_s
inter 1 2 lennard-jones $eps $sgm $Pr_c $Pc_s
inter 2 2 lennard-jones $eps $sgm $Pr_c $Pc_s
...
#VIRTUAL MOVING PARTICLE
if {$virt==yes} {
 part $NP pos [lindex [part 0 print pos] 0] [lindex [part 0 print pos] 1] [lindex [part 0 print pos] 2] type 5 bond 3 0
 incr NP 1
}
#MOVE
...
if {$virt==yes} {
 part $NP pos [lindex [part 0 print pos] 0] [lindex [part 0 print pos] 1] [lindex [part 0 print pos] 2]
}
###########################################
###########################################
########### PRESIMULATION #################
...
for { set krok 1 } { $krok <= 10 } { incr krok 1 } {
 ...
 if {$virt==yes} {
  part $NP pos [expr [lindex [part 0 print pos] 0]+$move*$m1/$m2] [lindex [part 0 print pos] 1] [lindex [part 0 print pos] 2]
 }
...
for { set krok 20 } { $krok <= $l1 } { incr krok 1 } {
 ...
 if {$virt==yes} {
  part $NP pos [expr [lindex [part 0 print pos] 0]+$move*$m1*10/$m2] [lindex [part 0 print pos] 1] [lindex [part 0 print pos] 2]
 }
 if { $vmd == "yes" } {writevcf $vtf_file folded}
}
#####################################################
...
#STRF
if {$t0==1} 				{set FILEstrf0   [open "FUNCTIONS/strf_0"   "w"]}
if {$t1==1} 				{set FILEstrf1   [open "FUNCTIONS/strf_1"   "w"]}
if {$t2==1} 				{set FILEstrf2   [open "FUNCTIONS/strf_2"   "w"]}
if {$t0==1 && $t2==1} 			{set FILEstrf02  [open "FUNCTIONS/strf_02"  "w"]}
if {$t0==1 && $t1==1 && $t2==1} 	{set FILEstrf012 [open "FUNCTIONS/strf_012" "w"]}
if {$t1==1 && $t2==1} 			{set FILEstrf12  [open "FUNCTIONS/strf_12"  "w"]}
#CORR
...
for { set krok 1 } { $krok <= $l2 } { incr krok 1 } {
 ...
 if {$virt==yes} {
  part $NP pos [expr [lindex [part 0 print pos] 0]+$move] [lindex [part 0 print pos] 1] [lindex [part 0 print pos] 2]
 }
 if { $vmd == "yes" } {writevcf $vtf_file folded}
...
#STRF
 if {$t0==1} 				{set strf [analyze structurefactor 0 100];if {$krok==1} {set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 0];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf0] $list}; set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 1];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf0] $list}
 if {$t1==1} 				{set strf [analyze structurefactor 1 100];if {$krok==1} {set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 0];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf1] $list}; set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 1];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf1] $list}
 if {$t2==1} 				{set strf [analyze structurefactor 2 100];if {$krok==1} {set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 0];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf2] $list}; set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 1];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf2] $list}
 if {$t0==1 && $t2==1} 		{set strf [analyze structurefactor {0 2} 100];if {$krok==1} {set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 0];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf02] $list}; set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 1];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf02] $list}
 if {$t0==1 && $t1==1 && $t2==1} 	{set strf [analyze structurefactor {0 1 2} 100];if {$krok==1} {set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 0];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf012] $list}; set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 1];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf012] $list}
 if {$t1==1 && $t2==1} 		{set strf [analyze structurefactor {1 2} 100];if {$krok==1} {set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 0];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf12] $list}; set list "";for {set i 1} {$i<100} {incr i} {set lnew [lindex $strf $i 1];if {$lnew!=""} {lappend list $lnew}};puts [set FILEstrf02] $list}
##################
...
## Rg ##
 ...
 puts $FILErg "$t [lindex [analyze rg 0 1 $N_B] 0] [lindex [analyze rg 0 1 $N_M] 0] [lindex [analyze rg $N_B 1 [expr $N_M-$N_B]] 0]"
## Re ##
...
close $FILEa_all
if {$t0==1} 				{close $FILEstrf0   }
if {$t1==1} 				{close $FILEstrf1   }
if {$t2==1} 				{close $FILEstrf2   }
if {$t0==1 && $t2==1} 			{close $FILEstrf02  }
if {$t0==1 && $t1==1 && $t2==1} 	{close $FILEstrf012 }
if {$t1==1 && $t2==1} 			{close $FILEstrf12  }
for { set i 0 } { $i <= 3 } { incr i } { if { [set t$i] == 1 } {
 close [set FILEpos$i]
