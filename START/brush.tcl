...
inter 0 1 lennard-jones $eps $sgm $r_c $c_s
inter 1 1 lennard-jones $eps $sgm $r_c $c_s
...
for { set i 1 } { $i < $N_B } { incr i 1 } {
 if { $i%2==1 && $N_A==0 } {
  part $i pos [expr $i*$r_0] 0.0 0.0 bond 0 [expr $i-1] q 1.0
 } else {
...
inter forcecap 0
for { set krok 11 } { $krok <= 1000 } { incr krok 1 } {
 puts "step $krok"
 integrate 80
 writevcf $vtf_file folded
}
inter forcecap 0
for { set krok 1001 } { $krok <= $steps } { incr krok 1 } {
 puts "step $krok"
 integrate 80
 writevcf $vtf_file folded
 #############
 ...
 for { set i 0 } { $i < [expr $N_B+($N_B+1)/$h*$N_A] } {incr i} {
  set xi [lindex [part $i print pos] 0]
  set yi [lindex [part $i print pos] 1]
  set zi [lindex [part $i print pos] 2]
  ...
  puts $afile "$krok $A11 $A21 $A31 $A12 $A22 $A32 $A13 $A23 $A33"
 ## 6 ##
 set first -1
 set sum6 0
 set div6 0
 ...
 puts $ifile "$krok $rg $re $sum3 $sum4 $sum6" 
}
exit
